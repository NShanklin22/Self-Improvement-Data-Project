...
def getDataByYear(df):
    ...
    while True:
        ...
        for i in range(len(years)):
            print(years[i])
        ReqYear = int(input('Please select a number for the corresponding year: '))
        # Check if the year provided is a valid month
        if ReqYear in years:
            YearData = df.where(df['date'].dt.year == ReqYear).dropna()
            if YearData.empty:
                print("There is no data for that year")
            else:
                print("Pulling data for {}".format(ReqYear))
                return YearData
        # If month is not valid ask the user for a new input
        else:
            print("{} is not a valid year #".format(ReqYear))